...
# read london boroughs lookup table file from github repository (so that
# filepath remains unchanged on any computer or folder, and there is no need to
# download)
...
# ------
# create a list of filename that corresponds to how the decs certificates are
# stored
...
# ----------------
# merge all the london boroughs decs certificates into 1 file by iteratively go
# through all the london borough folders stored on github

# define first and end parts of the file path that remain the same for each
# folder
path_start = 'https://raw.githubusercontent.com/LondonEnergyMap/rawdata/master/dec_london/display-'
path_end = '/certificates.csv'

# start by loading the first borough into dataframe df, by making a full
# filepath using filename of the borough
filename = boroughs[0]
filepath = path_start + filename + path_end
df = pd.read_csv(filepath)

# keep only entries of the latest date (to keep dataframe size small) by
# converting inspection date column to datetime format and dropping duplicate
# entries based on building reference and address, and keeping only last
# ascending date entry
